...
def split_test_train_dataset(res_health,yelp):
    ...
    """
        This function splits res_health (Nutritionix) dataset into test and train dataset
        It also creates a test dataset from yelp dataset to later generate predictions on
...
def vectorize_text(x_train, x_test, yelp_test):
    ...
    """
        This function invokes the CountVectorize from the library to vectorize data for model
        It returns the vecterization data for transformation for training, test and yelp data
    """
...
def fit_and_evaluate_model(training_data, test_data, x_test, label_col):
    ...
    """
        This function evaluates the model with training data against test data.
        It returns confusion matrix and accuracy score for model applied to test dataset
            - model: model object
...
def predict_on_Yelp(model, yelp_test_data, yelp):
    ...
    """
        This function takes fitted naive Bayes model as an input and predict on yelp dataset 
        It returns the final yelp dataset with prediction scores (healthy/unhealthy)
        It also generates CSV file that serves as an input to the UI application
    """
    ...
    yelp_final = yelp.groupby(['business_id','name', 'address','city','state','postal_code','latitude',
                               'longitude','stars','review_count','categories'])[['prediction_score']].mean()
        
    yelp_final['healthy_percent'] = pd.Series(["{0:.2f}%".format(val * 100)
                                               for val in yelp_final['prediction_score']], index = yelp_final.index)
