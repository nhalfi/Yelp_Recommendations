...
# Purpose: train and test text_classification model
...
# Declare all variables outside of the function main
# then reference them as global variables inside each function
...
def split_test_train_dataset(res_health, yelp):
    """
    This function splits res_health (Nutritionix) dataset into test and train
    dataset, also creates a test dataset from yelp dataset to later generate
    predictions
...
def vectorize_text(x_train, x_test, yelp_test):
    """
    - Function to invoke the CountVectorize from the library to
    vectorize data for model
    - Return the vecterization data for transformation for training,
     test and yelp data
    """
    ...
    # Perform vectorization on yelp dataset and generating predictions
    # by applying naive bayes classfier
...
def fit_and_evaluate_model(training_data, test_data, x_test, label_col):
    ...
    - model: model object
    - x_test: Nutritionix test dataset to calculate confusion
    ...
    - returns confusion matrix and accuracy score for model applied
        to test dataset
    """
...
def predict_on_Yelp(model, yelp_test_data, yelp):
    """
    - Function to take fitted naive Bayes model as an input
        and predict on yelp dataset
    - Returns the final yelp dataset with prediction scores of restaurent
        being healthy or not and generates CSV file that serves as an input
    ...
    # Generate final yelp dataset with predicted flags and average health
    ...
    yelp_final['healthy_percent'] = pd.Series(["{0:.2f}%".format(val * 100)
                                              ...
                                              ['prediction_score']],
                                              index=yelp_final.index)
    yelp_final

    # Export yelp predictions to CSV
    yelp_final.to_csv('../data/Yelp_Healthy_restaurant_predictions.csv',
                      header=True)
...
def main():
    try:
        ...
        training_data, test_data, yelp_test_data = vectorize_text(x_train,
                                                                  x_test,
                                                                  yelp_test)
        label_col = x_train['healthy']

        # Call the fit and evaluate function for Nutritionix
        conf_matrix, accuracy, model = fit_and_evaluate_model(training_data,
                                                              test_data,
                                                              x_test,
                                                              label_col)

        # Call the fit and evaluate function for Yelp
        predict_on_Yelp(model, yelp_test_data, yelp)
