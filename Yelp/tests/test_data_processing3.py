...
class test_data_processing(unittest.TestCase):
    ...
    # def test_calculate_health(self):
    #     df1 = pd.read_csv('logic_test.csv')
    #     df1_health = dp.calculate_health(df1)
    #     self.assertEqual(df1_health.loc[0:0, 'healthy']==0)
    #     self.assertEqual(df1_health.loc[1:1, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[2:2, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[3:3, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[4:4, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[5:5, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[6:6, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[7:7, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[8:8, 'healthy'] == 0)
    #     self.assertEqual(df1_health.loc[9:9, 'healthy'] == 1)


    def test_load_datasets(self):
        df1 = dp.load_datasets('../data/test_data.json','../data/test_photos.json', '../data/test_restaurants_items.json')
