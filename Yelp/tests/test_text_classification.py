...
class MyTestClassification(unittest.TestCase):
    ...
    def test_split_test_train_dataset(self):
        ...
        self.assertGreater(value_x_train.count()[0], 0, 'Fail: x_train sub-dataset')
        self.assertGreater(value_x_test.count()[0], 0, 'Fail: x_test sub-dataset')
        self.assertGreater(value_yelp_test.count()[0], 0, 'Fail: yelp_test sub-dataset')
    ...
    def test_vectorize_text(self):
        ...
        check_train, check_test, check_yelp = tc.vectorize_text(df_1, df_2, df_3)
        
        # data_1 has 5 words, so the vectorize value will be a matrix of 5 elements.
        # Because we vectorize (fit) x_train and we use that vector to transform the x_test and yelp_test 
        # Therefore, the matrixes of x_test and yelp_test will have the same size of 5, 
        ...
        # we know that vectorize library will arrange the word in alphabetical order from left to right
        ...
        # We can validate the same "apple" in data_2 will be in the same index position of "apple" in data_1
        print('Matrix index of apple value:', check_test.toarray()[0][1])
        self.assertEqual(check_test.toarray()[0][1], 1, 'Fail: x_test has different vectorized matrix index')
        ...
        self.assertEqual(check_yelp.toarray()[0][3], 1, 'Fail: yelp_test has different vectorized matrix index')
